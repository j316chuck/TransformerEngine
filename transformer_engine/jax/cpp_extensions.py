    ...
    def lowering(ctx, qkv, bias, cu_seqlen, seed, *, attn_bias_type, attn_mask_type, scaling_factor,
        ...
        *batch_shape, max_seqlen, _, num_head, head_dim = qkv_aval.shape
        ...
        opaque = transformer_engine_jax.pack_fused_attn_descriptor(
            batch_size, max_seqlen, max_seqlen, num_head, head_dim, wkspace_size,
            scaling_factor, dropout_probability, attn_bias_type, attn_mask_type,
...
class SelfFusedAttnBwdPrimitive(BasePrimitive):
    ...
    def abstract(qkv_aval, bias_aval, softmax_aux_aval, rng_state_aval, output_aval, doutput_aval,
        ...
        """
        del softmax_aux_aval, rng_state_aval, mask_or_cu_seqlen_aval
...
class CrossFusedAttnFwdPrimitive(BasePrimitive):
    ...
    def abstract(q_aval, kv_aval, bias_aval, q_mask_or_cu_seqlen_aval, kv_mask_or_cu_seqlen_aval,
        ...
        *q_batch_shape, q_max_seqlen, q_num_heads, q_head_dim = q_aval.shape
        *kv_batch_shape, kv_max_seqlen, nkv, kv_num_heads, kv_head_dim = kv_aval.shape
        ...
        # backend determines the softmax buffer shape/dtype
        backend = FusedAttnHelper(q_dtype, kv_dtype, NVTE_QKV_Layout.NVTE_BS2HD, attn_bias_type,
                                  attn_mask_type, dropout_probability, q_num_heads, kv_num_heads,
                                  q_max_seqlen, kv_max_seqlen, q_head_dim).get_fused_attn_backend()
        if backend == NVTE_Fused_Attn_Backend.NVTE_F16_max512_seqlen:
            softmax_shape = (*q_batch_shape, q_num_heads, q_max_seqlen, kv_max_seqlen)
            softmax_dtype = q_dtype
        elif backend == NVTE_Fused_Attn_Backend.NVTE_F16_arbitrary_seqlen:
            softmax_shape = (*q_batch_shape, q_num_heads, q_max_seqlen, 1)
            softmax_dtype = dtypes.canonicalize_dtype(jnp.float32)
        else:
            raise ValueError(f'Unsupported {backend=}')
        ...
        wkspace_info = transformer_engine_jax.get_cross_fused_attn_fwd_workspace_sizes(
            batch_size, q_max_seqlen, kv_max_seqlen, q_num_heads, q_head_dim,
            scaling_factor, dropout_probability, attn_bias_type, attn_mask_type,
    ...
    def lowering(ctx, q, kv, bias, q_cu_seqlen, kv_cu_seqlen, seed, *, attn_bias_type,
        ...
        *batch_shape, q_max_seqlen, q_num_heads, q_head_dim = q_aval.shape
        ...
        opaque = transformer_engine_jax.pack_fused_attn_descriptor(
            batch_size, q_max_seqlen, kv_max_seqlen, q_num_heads, q_head_dim, wkspace_size,
            scaling_factor, dropout_probability, attn_bias_type, attn_mask_type,
...
class CrossFusedAttnBwdPrimitive(BasePrimitive):
    ...
    def lowering(ctx, q, kv, bias, softmax_aux, rng_state, output, doutput, q_cu_seqlen,
        ...
        *batch_shape, q_max_seqlen, num_head, head_dim = q_aval.shape
        ...
        opaque = transformer_engine_jax.pack_fused_attn_descriptor(
            batch, q_max_seqlen, kv_max_seqlen, num_head, head_dim, wkspace_size,
            scaling_factor, dropout_probability, attn_bias_type, attn_mask_type,
            jax_dtype_to_te_dtype(q_aval.dtype), jax_dtype_to_te_dtype(wkspace_aval.dtype),
            is_training)
