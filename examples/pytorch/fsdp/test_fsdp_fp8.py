...
parser.add_argument('-m', "--module", type=lowercase, default='transformerlayer',
...
parser.add_argument('-d', "--head-dim", type=int, default=256,
...
dtype = args.dtype
seq_len = args.sequences
batch_size = args.batch
num_heads = args.heads
head_dim = args.channels
...
if args.layers > 1:
    ...
    for i in range(args.layers):
        ...
        te_layer_list.append(new_layer.to(dtype=dtype).cuda())
    te_model = nn.Sequential(*te_layer_list)
else:
    te_model = get_layer(layer_type)
    te_model.to(dtype=dtype).cuda()
...
print(f"\n[GPU-{local_rank}] Training Time: {train_time}s\n" +
        f"[GPU-{local_rank}] Avg. Iter. Time: {train_time /args.iters}s\n" +
