...
parser.add_argument("--defer-init", action="store_true",
                    help="Use torchdistx.deferred_init to delay module initialization until after the FSDP wrapper.")
...
def get_layer(layer_type, layer_number=None):
    if layer_type == 'linear':
        ...
        layer_cls = te.TransformerLayer
        layer_args = (hidden_size, ffn_hidden_size, num_heads)
        layer_kwargs = {
    ...
    return layer_cls(*layer_args, **layer_kwargs)
...
if args.num_layers > 1:
    ...
    te_layer_list = [ get_layer(layer_type, layer_number=i+1) for i in range(args.num_layers) ]
    te_model = nn.Sequential(*te_layer_list)
else:
    te_model = get_layer(layer_type)
